...
const GetLegendByID = () => {
  const [legendId, setLegendId] = useState("4"); // Initial value
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `https://brawlhalla.fly.dev/v1/legends/id?legend_id=${legendId}`
      );
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex flex-col items-center p-6">
        <h1 className="text-2xl font-bold mb-4">Get Legend By ID</h1>
      </div>
      <Input
        label="Legend ID"
        bordered

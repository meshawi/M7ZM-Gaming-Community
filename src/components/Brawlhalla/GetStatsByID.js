  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `https://brawlhalla.fly.dev/v1/stats/id?brawlhalla_id=${brawlhallaId}`
      );
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex flex-col items-center p-6">
        <h1 className="text-2xl font-bold mb-4">{t('getStatsByID')}</h1>
      </div>
      <Input
        ...
        <Card className="w-full max-w-md mt-4">
          <CardHeader className="flex gap-3">
            <div className="flex flex-col">
              ...
              <p className="p-md">{t('level')}: {data.level}</p>
              <p className="p-md">{t('xp')}: {data.xp}</p>
              <p className="p-md">{t('games')}: {data.games}</p>
              <p className="p-md">{t('wins')}: {data.wins}</p>
              <p className="p-md">{t('clan')}: {data.clan.clan_name}</p>
            </div>
          </CardHeader>
          <Divider />
          <CardBody>
            ...
            <Accordion variant="shadow">
              {currentData.map((legend, index) => (
                ...
                >
                  <p>{t('damageDealt')}: {legend.damagedealt}</p>
                  <p>{t('damageTaken')}: {legend.damagetaken}</p>
                  <p>{t('kos')}: {legend.kos}</p>
                  <p>{t('falls')}: {legend.falls}</p>
                  <p>{t('suicides')}: {legend.suicides}</p>
                  <p>{t('teamKos')}: {legend.teamkos}</p>

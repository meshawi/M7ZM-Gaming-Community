  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `https://brawlhalla.fly.dev/v1/utils/rankedseasonal?region=eu&page=1`
      );
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex flex-col items-center p-6">
        <h1 className="text-2xl font-bold mb-4">{t('getRankedSeasonalData')}</h1>
      </div>
      <Button
        shadow
        color="primary"
      ...
      {data && (
        <Table
          aria-label={t('rankedSeasonalDataTable')}
          css={{
            height: "auto",
            minWidth: "100%",
          }}
...
export default GetRankedSeasonalData;
